...
fn field_to_py(py: Python<'_>, field: &NestedField) -> PyResult<Py<PyAny>> {
    ...
    // Expose Iceberg types as canonical spec JSON instead of duplicating
    // PyIceberg's type hierarchy in this binding.
    d.set_item(
        "type",
...
impl PySchema {
    ...
    fn _capsule<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyCapsule>> {
        // Internal handoff for follow-on Rust-backed bindings that need the
        // parsed Schema without reparsing JSON.
